...
class TrelloWebhooksController < ApplicationController
  ...
  def update_redmine_issue
    ...
    unless @webhooks_request.valid_request?
      ...
      render status: 403, json: { status: 'error', message: 'Mismatched signature, maybe a fake reqest.' } and return true
    end
    ...
    if @webhooks_request.payload['action']['type'] == 'updateCard'
      ...
      if issue.nil?
        render status: 500, json: { status: 'error', message: 'Unable to find the Redmine issue.' } and return true
      end

      # TODO: comment out this debug info
      ...
      if @webhooks_request.payload['action']['data']['old']['due']
        ...
        if issue.due_date != new_due
          issue.due_date = Date.parse(new_due)
        end
      end
      ...
      if @webhooks_request.payload['action']['data']['old']['idList']
        ...
        new_stat_id = list_mapping.select { |k,v| v == new_list_id }.keys.first
